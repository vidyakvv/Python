"""
Given a binary matrix A, we want to flip the image horizontally, then invert it, and return the resulting image.
To flip an image horizontally means that each row of the image is reversed.  For example, flipping [1, 1, 0]
horizontally results in [0, 1, 1].
To invert an image means that each 0 is replaced by 1, and each 1 is replaced by 0. For example, inverting [0, 1, 1]
results in [1, 0, 0].
Input: [[1,1,0],[1,0,1],[0,0,0]]
Output: [[1,0,0],[0,1,0],[1,1,1]]
Explanation: First reverse each row: [[0,1,1],[1,0,1],[0,0,0]].
Then, invert the image: [[1,0,0],[0,1,0],[1,1,1]]
"""
from typing import List
class Solution:
    def flipAndInvertImage(self, A: List[List[int]]) -> List[List[int]]:
        #my_list = [[0 for col in range(len(A[0]))] for row in range(len(A))]
        #print(my_list)
        for each_list in A:
            range_len = int((len(each_list)+1)/2)
            print(range_len)
            for i in range(range_len):
                """ each_list.reverse()
                for i in range(len(each_list)):
                if each_list[i] == 1:
                    each_list[i] = 0
                else:
                    each_list[i] = 1"""
                each_list[i], each_list[~i] = each_list[~i] ^ 1, each_list[i] ^ 1
        return A

if __name__ == "__main__":
    my_solution = Solution()
    print(my_solution.flipAndInvertImage([[1,1,0],[1,0,1],[0,0,0]]))